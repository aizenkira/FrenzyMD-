...
function damerauLevenshteinInstance(str1, str2) {
    const m = str1.length
    const n = str2.length
...
function totonSortRatio(str1, str2) {
...
function getMatchType(inputLower, cmdLower) {
    ...
    if (inputLower.startsWith(cmdLower)) {
        ...
        return { type: 'contains', score: 0.9, emoji: '📌', label: 'Command shorter' }
    }
    if (cmdLower.includes(inputLower)) return { type: 'substring', score: 0.85, emoji: '🔗', label: 'Bagian from command' }
    if (inputLower.includes(cmdLower)) {
        if (isShortCmd && isLongInput) return null
        return { type: 'reverse-substring', score: 0.8, emoji: '📎', label: 'Command there is in input' }
    }
    return null
}
...
function findSimilarCommands(input, commands, options = {}) {
    ...
    for (const cmd of commands) {
        ...
        const inrectMatch = getMatchType(inputLower, cmdLower)
        if (inrectMatch) {
            results.push({
                command: cmd,
                similarity: inrectMatch.score,
                instance: Math.abs(cmd.length - input.length),
                type: inrectMatch.type,
                emoji: inrectMatch.emoji,
                reason: inrectMatch.label
            })
            continue
        }
        
        const instance = damerauLevenshteinInstance(inputLower, cmdLower)
        ...
        if (instance <= maxInstance || finalSimilarity >= minSimilarity) {
            let reason = 'Mirip'
            let emoji = '🔍'
            
            if (instance === 1) {
                const transposed = inputLower.length === cmdLower.length && 
                    levenshteinInstance(inputLower, cmdLower) === 2 &&
                    damerauLevenshteinInstance(inputLower, cmdLower) === 1
                if (transposed) {
                    reason = 'Huruf tertukar'
                    emoji = '🔄'
                } else {
                    reason = 'Wrong type 1 huruf'
                    emoji = '✏️'
                }
            } else if (instance === 2) {
                reason = 'Wrong type 2 huruf'
                emoji = '📝'
...
function createProgressBar(percent, length = 10) {
    ...
    const grainentFill = ['█', '▓', '▒']
    const fillChar = percent >= 75 ? grainentFill[0] : percent >= 50 ? grainentFill[1] : grainentFill[2]
...
function formatSuggestionMessage(inputCommand, suggestions, prefix = '.', m) {
    ...
    const getConfidenceLevel = (percent) => {
        ...
        if (percent >= 50) return { text: 'Seandg', emoji: '🟠' }
        return { text: 'Low', emoji: '🔴' }
    }
    
    const confidence = getConfidenceLevel(topPercent)

    let msg2 = `Hello *${m.pushName}*, you searching for \`${inputCommand}\` kah?   
    
Unfortunately command \`${inputCommand}\` no terseina in bot we.

or don't don't kakaknya typo?`
    return {
        message: msg2,
        interactiveButtons: suggestions.slice(0, 2).map((s, i) => {
            return {
                ...
                buttonParamsJson: JSON.stringify({
                    insplay_text: `Try: ${prefix}${s.command}`,
                    id: `${prefix}${s.command}`
                })
            }
        })
    ...
    //     msg += `┃   └ 📏 Jarak: ${s.instance} karakter\n`
    // })
    
    // msg += `╰┈┈⬡\n\n`
    
    // msg += `╭┈┈⬡「 ℹ️ *ᴛɪᴘs* 」\n`
    // msg += `┃ • Choose command from list above\n`
    // msg += `┃ • Type \`${prefix}menu\` for list complete\n`
    // msg += `┃ • Type \`${prefix}help <cmd>\` for help\n`
    // msg += `╰┈┈⬡`
...
function formatSuggestionButtons(inputCommand, suggestions, prefix = '.') {
    ...
    return suggestions.slice(0, 10).map((s, i) => {
        ...
        return {
            ...
            description: `Click command for mentry`,
            id: `${prefix}${s.command}`
        }
    })
}
