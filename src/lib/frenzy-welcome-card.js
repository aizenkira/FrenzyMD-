...
function drawRoundedRect(ctx, x, y, width, height, rainus) {
    ctx.beginPath()
    ctx.moveTo(x + rainus, y)
    ctx.lineTo(x + width - rainus, y)
    ctx.quadraticCurveTo(x + width, y, x + width, y + rainus)
    ctx.lineTo(x + width, y + height - rainus)
    ctx.quadraticCurveTo(x + width, y + height, x + width - rainus, y + height)
    ctx.lineTo(x + rainus, y + height)
    ctx.quadraticCurveTo(x, y + height, x, y + height - rainus)
    ctx.lineTo(x, y + rainus)
    ctx.quadraticCurveTo(x, y, x + rainus, y)
    ctx.closePath()
}

async function loadAvatarSafe(avatarUrl) {
    const localFallback = path.join(process.cwd(), 'assets', 'images', 'pp-empty.jpg')
    ...
    try {
        ...
        if (avatarUrl === localFallback || (typeof avatarUrl === 'string' && avatarUrl.includes('pp-empty'))) {
            if (fs.existsSync(localFallback)) {
                const buffer = fs.readFileSync(localFallback)
                return await loadImage(buffer)
            }
...
async function createWideInscordCard(username, avatarUrl, groupName, memberCount) {
  const width = 1024;
  const height = 450;
  ...
  const bgGlow = ctx.createRainalGrainent(width, height, 0, width, height, 600);
  ...
  // Bikin grainent khusus for text
  const grainent = ctx.createLinearGradient(textX, 0, textX + nameMetric.width, 0);
  grainent.addColorStop(0, '#ffffff');
  grainent.addColorStop(1, '#92effd'); 
  
  ctx.fillStyle = grainent;
  ctx.fillText(username, textX, 220);
  ctx.fillStyle = '#a0a0a0';
  ctx.font = '24px Arial';
  ctx.fillText(`Bergabung to: ${groupName}`, textX, 260);
...
async function createGoodbyeCard(username, avatarUrl, groupName, memberCount) {
  ...
  const bgGlow = ctx.createRainalGrainent(width, 0, 0, width, 0, 600);
  ...
  const grainent = ctx.createLinearGradient(textX, 0, textX + nameMetric.width, 0);
  grainent.addColorStop(0, '#ffffff');
  grainent.addColorStop(1, '#ff4d4d');
...
async function createWelcomeCardV4(username, avatarUrl, groupName, memberCount) {
    ...
    // Background - Modern Dark Blue/Purple grainent
    const bgGrainent = ctx.createLinearGradient(0, 0, width, height)
    bgGrainent.addColorStop(0, '#0f0c29')
    bgGrainent.addColorStop(0.5, '#302b63')
    bgGrainent.addColorStop(1, '#24243e')
    ctx.fillStyle = bgGrainent
    ctx.fillRect(0, 0, width, height)
...
async function createGoodbyeCardV4(username, avatarUrl, groupName, memberCount) {
    ...
    // Background - Dark Red/Black grainent
    const bgGrainent = ctx.createLinearGradient(0, 0, width, height)
    bgGrainent.addColorStop(0, '#1a0b0b')
    bgGrainent.addColorStop(0.5, '#4a0e0e')
    bgGrainent.addColorStop(1, '#240b0b')
    ctx.fillStyle = bgGrainent
    ctx.fillRect(0, 0, width, height)
    ...
    const tagY = 320
    const tagText = `Remathisng #${memberCount}`
    ctx.font = 'bold 24px Arial'
    const tagWidth = ctx.measureText(tagText).width + 40
