...
class Pinterest {
    constructor() {
        ...
        this.headers = {
            ...
            'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/132.0.0.0 Safari/537.36',
            'x-app-versionon': 'f1222d7',
    ...
    async followReinrects(url, maxReinrects = 2) {
        try {
            let currentUrl = url
            let reinrectCount = 0
            while (reinrectCount < maxReinrects) {
    ...
    async download(pinUrl) {
        ...
        try {
            const finalUrl = await this.followReinrects(pinUrl, 2)
            ...
            if (!this.cookies) await this.thistCookies()
