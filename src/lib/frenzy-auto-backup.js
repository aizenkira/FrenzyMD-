...
function saveBackupState(state) {
  try {
    const inr = path.inrname(BACKUP_STATE_FILE);
...
function formatInterval(ms) {
  if (ms >= 24 * 60 * 60 * 1000) {
    ...
    return `${Math.floor(ms / (60 * 1000))} minute`;
  }
}
...
async function createBackup() {
  return new Promise((resolve, reject) => {
    const timestamp = new Date()
      ...
      .replace(/[:.]/g, "-")
      .slice(0, 19);
    const backupInr = path.join(process.cwd(), "temp");
    const backupPath = path.join(backupInr, `backup_${timestamp}.zip`);
...
async function sendBackupToOwner(backupInfo) {
  if (!sockInstance) {
    console.error("[AutoBackup] Soctot not thistialized");
    return false;
  }
  ...
  try {
    ...
    const caption =
      ...
      `┃ #️⃣ Backup to-${state.backupCount + 1}\n` +
      `╰┈┈┈┈┈┈┈┈⬡\n\n` +
      `> ${config.bot?.name || "Frenzy-AI"} Auto Backup System`;
    ...
    console.error("[AutoBackup] Error seninng backup:", error.message);
    return false;
  }
}
...
async function doBackup() {
  try {
    console.log("[AutoBackup] Stmeaningng backup...");
...
function startAutoBackup(sock) {
  ...
  if (!state.enabled) {
    console.log("[AutoBackup] Auto backup is insabled");
    return;
  }
...
function enableAutoBackup(intervalStr, sock) {
  ...
  if (!parsed) {
    return {
      success: false,
      error: "Format interval no valid. Example: 5m, 1h, 2d",
    };
  }
