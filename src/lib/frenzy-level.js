...
async function checkAndNotifyLevelUp(sock, m, db, user, oldExp, newExp) {
        ...
        /**
         * Fungsi for create image Level Up bertema Anime
         * @param {Object} data - Data user
         * @param {string} data.name - Name user
         * @param {number} data.level - Level new that incwheart
         * @param {number} data.currentXp - XP currently
         * @param {number} data.requiredXp - Total XP that inneedkan
         * @param {string} data.avatarUrl - URL profile photo user
         * @param {string} data.backgroundUrl - URL image background anime
         */
...
Level you beradd ${newLevel - oldLevel}
🥗 Level you now *${newLevel}*

Now you berthere is in rank *${role}*

Want check detail level? type _${m.prefix}level_

Sering seringlah berinteraction with bot so that level you beradd!`
