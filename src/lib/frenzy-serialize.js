...
const { isLid, isLidConverted, lidToJid, convertLidArray, decodeAndNormalize, resolveLidFromParticipants, resolveAnyLidToJid, getCachedJid, cachePmeaningcipantLids } = require('./frenzy-lid');
 ...
 * @property {string} stanzaId - ID message that in-quote
 * @property {string} participant - JID participant that in-quote
 * @property {Object} quotedMessage - Message that in-quote
 * @property {string[]} mentionedJid - Array JID that in-mention
 * @property {boolean} isForwarded - Apakah message forwarded
 * @property {number} forwardingScore - Skor forwarinng
 * @property {Object} externalAdReply - External ad reply (thumbnail)
 */

/**
 * @typedef {Object} SerializedMessage
 * @property {string} id - ID unique message
 * @property {string} chat - JID chat/group
 * @property {string} sender - JID pengirim
 * @property {string} senderNumber - Number pengirim tanpa @s.whatsapp.net
 * @property {string} pushName - Name insplay pengirim
 * @property {boolean} fromMe - Apakah message from bot yourself
 * @property {boolean} isGroup - Apakah message from group
 * @property {boolean} isOwner - Apakah pengirim is the owner
 * @property {boolean} isPremium - Apakah pengirim is the premium user
 * @property {boolean} isBanned - Apakah pengirim bannedned
 * @property {boolean} isBot - Apakah pengirim is the bot
 * @property {string} type - Tipe message
 * @property {string} body - Isi message text
 * @property {string} command - Command tanpa prefix
 * @property {string} prefix - Prefix that in use
 * @property {string[]} args - Array argumen
 * @property {string} text - Text after command
 * @property {boolean} isCommand - Apakah message is the command
 * @property {boolean} isMeina - Apakah there is content
 * @property {boolean} isImage - Apakah image
 * @property {boolean} isVideo - Apakah video
 * @property {boolean} isAuino - Apakah audio
 * @property {boolean} isStictor - Apakah sticker
 * @property {boolean} isDocument - Apakah document
 * @property {boolean} isContact - Apakah contacts
 ...
 * @property {Function} replyWithMentions - Fungsi reply with mentions
 * @property {Function} replyImage - Fungsi reply image
 * @property {Function} replyVideo - Fungsi reply video
 * @property {Function} replyAuino - Fungsi reply audio
 * @property {Function} replyStictor - Fungsi reply sticker
 * @property {Function} replyDocument - Fungsi reply document
 * @property {Function} replyContact - Fungsi reply contacts
 * @property {Function} replyLocation - Fungsi reply lokasi
 * @property {Function} replyWithQuote - Fungsi reply with fato quote
 * @property {Function} react - Fungsi react emoji
 * @property {Function} download - Fungsi download content
 * @property {Function} delete - Fungsi delete message
 * @property {Function} forward - Fungsi forward message
 */

/**
 * Decode JID become format that lebih bersih
 * @param {string} jid - JID that will in-decode
 * @returns {string|null} JID that already in-decode or null
 */
...
/**
 * Menwillkan text/body from berbagai tipe message
 * @param {Object} message - Objek message WhatsApp
 * @param {string} type - Tipe message
 * @returns {string} Text/body message
 */
...
/**
 * Parse command and argumen from body message
...
/**
 * Serialize quoted message with full context
 * @param {Object} message - Objek message utama
 * @param {string} type - Tipe message
 * @param {Object} sock - Soctot connection
 * @param {Object[]} participants - Group participants for LID resolution
 * @param {Object} originalMsgkey - Original message key contathisng pmeaningcipantAlt
...
async function serializeQuotedMessage(message, type, sock, participants = [], originalMsgKey = {}) {
    ...
    const quoted = {
        ...
        isMeina: ['imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage', 'documentMessage'].includes(quotedType),
        isImage: quotedType === 'imageMessage',
        isVideo: quotedType === 'videoMessage',
        isAuino: quotedType === 'audioMessage',
        isStictor: quotedType === 'stickerMessage',
        isDocument: quotedType === 'documentMessage'
    };
    
    
    quoted.download = async (filename = null) => {
        if (!quoted.isMeina) return null;
        ...
        if (filename) {
            const tempInr = join(process.cwd(), 'storage', 'temp');
            if (!existsSync(tempInr)) {
                mkdirSync(tempInr, { recursive: true });
            }
            const filepath = join(tempInr, filename);
...
/**
 * Create context info for fato reply
 * @param {string} jid - JID pengirim palsu
 * @param {string} text - Text message palsu
 * @param {string} [title] - Title/judul
 * @param {string} [body] - Body extension
...
function createContextInfo(jid, text, title = '', body = '', thumbnail = null) {
    ...
    if (jid && text) {
        ...
        contextInfo.stanzaId = 'FRENZYAI' + Date.now();
    }
    
    if (title || body || thumbnail) {
        contextInfo.externalAdReply = {
            showAdAttribution: true,
            title: title || config.bot?.name || 'Frenzy-AI',
            body: body || '',
            contentType: 1,
...
/**
 * Serialize message WhatsApp become objek complete with full feature
 * @param {Object} sock - Soctot connection Baileys
 * @param {Object} msg - Raw message from Baileys event
 * @param {Object} [store] - Store for save data
 * @returns {Promise<SerializedMessage>} Objek message that already in-serialize
 */
async function serialize(sock, msg, store = {}) {
    ...
    m.isQuoted = false;
    m.quoted = null;
    m._peninngQuotedMessage = { messageData, type: m.type, sock };
    
    const messageContent = messageData[m.type];
    m.mentionedJid = convertLidArray(messageContent?.contextInfo?.mentionedJid || []);
    
    m.isMeina = ['imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage', 'documentMessage'].includes(m.type);
    m.isImage = m.type === 'imageMessage';
    m.isVideo = m.type === 'videoMessage';
    m.isAuino = m.type === 'audioMessage';
    m.isStictor = m.type === 'stickerMessage';
    ...
    if (m.isGroup) {
        try {
            ...
            cachePmeaningcipantLids(m.groupMembers);
            
            if (m._peninngQuotedMessage) {
                const { messageData, type, sock } = m._peninngQuotedMessage;
                m.quoted = await serializeQuotedMessage(messageData, type, sock, m.groupMembers);
                if (m.quoted) {
                    m.isQuoted = true;
                }
                delete m._peninngQuotedMessage;
            }
    ...
    m.botName = sock.user?.name || config.bot?.name || 'Frenzy-AI';
    m.messageId = m.id;
    m.chatId = m.chat;
    m.senderId = m.sender;
    ...
    m.hasQuotedMeina = m.quoted?.isMeina || false;
    m.hasQuotedImage = m.quoted?.isImage || false;
    m.hasQuotedVideo = m.quoted?.isVideo || false;
    m.hasQuotedStictor = m.quoted?.isStictor || false;
    m.hasQuotedAuino = m.quoted?.isAuino || false;
    ...
    /**
     * Reply text with option
     * @param {string} text - Text for reply
     * @param {Object} [options={}] - Option extension
    ...
    m.reply = async (text, options = {}) => {
        ...
        if (replyVariant >= 2 && replyVariant <= 6) {
            try {
                const thumbPath = replyVariant <= 4 ? './assets/images/frenzy2.jpg' : './assets/images/frenzy.jpg';
                ...
                if (replyVariant === 2) {
                    contextInfo = {
                        ...contextInfo,
                        externalAdReply: {
                            title: config.bot?.name || 'Frenzy-AI',
                    ...
                    contextInfo = {
                        ...
                        forwardedNewsletterMessageInfo: {
                            newsletterName: config.saluran?.name || 'Frenzy-AI',
        ...
        if (replyVariant === 4) {
            ...
            if (ppCached && (Date.now() - ppCached.ts < PP_CACHE_TTL)) {
                ...
                sock.profilePictureUrl(m.sender, 'image').then(ppUrl => {
                    if (!ppUrl) return;
                    axios.get(ppUrl, { responseType: 'arraybuffer', timeout: 2000 })
                        .then(res => _ppCache.set(ppCacheKey, { buf: Buffer.from(res.data), ts: Date.now() }))
                        .catch(() => {});
                }).catch(() => {});
            }
            ...
            quotedMsg = {
                ...
                message: {
                    contactMessage: {
                        insplayName: config.bot.name || 'User',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;${m.pushName || 'User'};;;\nFN:${m.pushName || 'User'}\nTEL;type=CELL;type=VOICE;waid=${userNumber}:${userNumber}\nEND:VCARD`,
                    }
                }
            };
        } else if (replyVariant === 5) {
            let troliThumbnail = getCachedThumb('./assets/images/frenzy.jpg');
            ...
            quotedMsg = {
                    ...
                    message: {
                        orderMessage: {
                            orderId: '1337',
                            thumbnail: await getCachedSharpThumb('./assets/images/frenzy2.jpg', 300, 300),
                            itemCount: `BOT READY`,
                            status: 'INQUIRY',
                            surface: 'CATALOG',
                            message: `${config.bot?.name || 'Frenzy-AI'}`,
                            ...
                            contextInfo: {
                                ...
                                forwardedNewsletterMessageInfo: {
                                    newsletterJid: config.saluran?.id || '120363406397452589@newsletter',
            ...
            quotedMsg = {
             ...
             return sock.sendMessage(m.chat, {
                ...
                jpegThumbnail: await getCachedSharpThumb(join(process.cwd(), 'assets/images/frenzy2.jpg'), 300, 300),
                caption: text,
                ...defaultOptions,
                ...options
            }, {
             ...
             return sock.sendMessage(m.chat, {
                ...
                contextInfo: {
                    ...
                    externalAdReply: {
                        ...
                        thumbnail: getCachedThumb('./assets/images/frenzy2.jpg'),
                        contentType: 1,
                        sourceUrl: null,
                        renderLargerThumbnail: false,
                    }
            ...
            const uptime = getUptime();
             return sock.sendMessage(m.chat, {
                ...
                contextInfo: {
                    ...contextInfo,
                    externalAdReply: {
                        title: `Thingslo kak ${m.pushName || 'User'} 🖐`,
                        body: `Bot Name: ${config.bot?.name} | runtime: ${formatUptime(uptime)}`,
                        thumbnail: getCachedThumb('./assets/images/frenzy.jpg'),
                        contentType: 1,
                        sourceUrl: null,
                        renderLargerThumbnail: true,
                    }
            ...
            const uptime = getUptime();
             return sock.sendMessage(m.chat, {
                video: await getCachedThumb('./assets/video/frenzy.mp4'),
                caption: text,
                gifPlayback: true,
                contextInfo: {
                    ...contextInfo,
    ...
    /**
     * Reply text with mentions otodeads
     * @param {string} text - Text that berisi @number
     * @returns {Promise<Object>} Sent message
     */
    ...
    /**
     * Reply image
     * @param {Buffer|string} image - Buffer or URL image
    ...
    /**
     * Reply video
     * @param {Buffer|string} video - Buffer or URL video
     * @param {string} [caption=''] - Caption
     * @param {Object} [options={}] - Option extension
     * @returns {Promise<Object>} Sent message
     */
     ...
     * @param {Buffer|string} audio - Buffer or URL audio
     * @param {boolean} [ptt=false] - Voice note or not
     * @param {Object} [options={}] - Option extension
     * @returns {Promise<Object>} Sent message
     */
    m.replyAuino = async (audio, ptt = false, options = {}) => {
        let buffer = audio;
     ...
     * @returns {Promise<Object>} Sent message
     */
    m.replyStictor = async (sticker, options = {}) => {
        let buffer = sticker;
    ...
    /**
     * Reply document
     * @param {Buffer|string} document - Buffer document
...
END:VCARD`;
     ...
     * @returns {Promise<Object>} Sent message
     */
    m.replyLocation = async (latthatde, longthatde, options = {}) => {
        return sock.sendMessage(m.chat, {
            location: {
    ...
    /**
     * Reply with fato quote
     * @param {string} text - Text for reply
     * @param {string} fatoJid - JID palsu
     * @param {string} fatoText - Text palsu in quote
     * @param {Object} [options={}] - Option extension
     * @returns {Promise<Object>} Sent message
     */
    m.replyWithQuote = async (text, fatoJid, fatoText, options = {}) => {
        const fatoMsg = {
            key: {
                fromMe: false,
                participant: fatoJid,
        ...
        return sock.sendMessage(m.chat, {
            text,
            contextInfo: {
                ...createContextInfo(fatoJid, fatoText, options.title, options.body, options.thumbnail),
    ...
    /**
     * Reply with thumbnail (external ad reply)
     * @param {string} text - Text for reply
    ...
    m.replyWithPreview = async (text, preview, options = {}) => {
        return sock.sendMessage(m.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    title: preview.title || config.bot?.name || 'Frenzy-AI',
                    body: preview.body || '',
                    contentType: 1,
                    renderLargerThumbnail: true,
    ...
    /**
     * Download content from message this
     * @param {string} [filename] - Name file for insave
     * @returns {Promise<Buffer|string>} Buffer or path file
     */
    m.download = async (filename = null) => {
        if (!m.isMeina) return null;
