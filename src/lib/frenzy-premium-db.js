...
function addOwner(jid, name = 'Unknown') {
    const cleanJid = jid?.replace(/@.+/g, '') || ''
    if (isOwner(cleanJid)) return { success: false, message: 'Already become owner' }
    ...
    saveOwners(owners)
    return { success: true, message: 'Success added as owner' }
}
...
function removeOwner(jid) {
    ...
    if (index === -1) return { success: false, message: 'Not found in list owner' }

    owners.splice(index, 1)
    saveOwners(owners)
    return { success: true, message: 'Success deleted from owner' }
}
...
function addPremium(jid, days = 30, name = 'Unknown') {
    ...
    if (existing !== -1) {
        ...
        return { success: true, message: `Premium extended ${days} day`, expiredAt: premiumList[existing].expiredAt }
    }
    ...
    savePremium(premiumList)
    return { success: true, message: `Success added as premium ${days} day`, expiredAt }
}
...
function removePremium(jid) {
    ...
    if (index === -1) return { success: false, message: 'Not found in list premium' }

    premiumList.splice(index, 1)
    savePremium(premiumList)
    return { success: true, message: 'Success deleted from premium' }
}
...
function addPartner(jid, days = 30, name = 'Unknown') {
    ...
    if (existing !== -1) {
        ...
        return { success: true, message: `Partner extended ${days} day`, expiredAt: partnerList[existing].expiredAt }
    }
    ...
    savePartners(partnerList)
    return { success: true, message: `Success added as partner ${days} day`, expiredAt }
}
...
function removePartner(jid) {
    ...
    if (index === -1) return { success: false, message: 'Not found in list partner' }

    partnerList.splice(index, 1)
    savePartners(partnerList)
    return { success: true, message: 'Success deleted from partner' }
}
