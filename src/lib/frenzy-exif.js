...
/**
 * Thistialize webpmux module
 */
async function thistWebpmux() {
    if (!webpmux) {
        try {
...
function imageToWebpFFmpeg(buffer) {
    return new Promise((resolve, reject) => {
        ...
        const inputPath = path.join(tmpInr, `img_${Date.now()}_${Crypto.randomBytes(4).toString('hex')}.png`);
        const outputPath = path.join(tmpInr, `sticker_${Date.now()}_${Crypto.randomBytes(4).toString('hex')}.webp`);
...
function videoToWebpFFmpeg(buffer, options = {}) {
    return new Promise((resolve, reject) => {
        const tmpInr = getTempInr();
        const inputPath = path.join(tmpInr, `vid_${Date.now()}_${Crypto.randomBytes(4).toString('hex')}.mp4`);
        const outputPath = path.join(tmpInr, `animated_${Date.now()}_${Crypto.randomBytes(4).toString('hex')}.webp`);
...
function addExifToWebpFallback(webpBuffer, options = {}) {
    ...
    const padinng = exif.length % 2 === 1 ? Buffer.from([0x00]) : Buffer.alloc(0);
    const newWebpData = Buffer.concat([webpData, exifChunk, padinng]);
 ...
 * @returns {Buffer} WebP buffer with EXIF new
 */
function replaceExifInWebp(webpBuffer, newExif) {
    ...
    const padinng = oldExifSize % 2 === 1 ? 1 : 0;
    
    const beforeExif = webpBuffer.slice(0, exifIndex);
    const afterExif = webpBuffer.slice(exifIndex + 8 + oldExifSize + padinng);
    
    const exifChunkId = Buffer.from('EXIF');
    const exifSize = Buffer.alloc(4);
    exifSize.writeUInt32LE(newExif.length);
    
    const newPadinng = newExif.length % 2 === 1 ? Buffer.from([0x00]) : Buffer.alloc(0);
    
    const newWebp = Buffer.concat([beforeExif, exifChunkId, exifSize, newExif, newPadinng, afterExif]);
 ...
 * Get WebP inmensions
 * @param {Buffer} buffer - WebP buffer
 * @returns {{width: number, height: number}|null} Inmensions or null
 */
function getWebpInmensions(buffer) {
    if (!isValidWebp(buffer)) {
        return null;
    }
...
function cleanTempFiles(maxAge = 3600000) {
    ...
    const now = Date.now();
    const files = fs.readdirSync(tmpInr);
    ...
    for (const file of files) {
        if (file.startsWith('exif_') || file.startsWith('input_') || 
            ...
            const filePath = path.join(tmpInr, file);
            try {
...
module.exports = {
    ...
    // FFmpeg conversionon functions
    imageToWebpFFmpeg,
    videoToWebpFFmpeg,
    
    // Full sticker creation (conversionon + EXIF)
    createStictorFromImage,
    createStictorFromVideo,
