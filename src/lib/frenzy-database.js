...
class Database {
    constructor(dbPath) {
        this.dbPath = dbPath;
        this.stores = {};
        this.inrty = { users: false, groups: false, settings: false, stats: false, sewa: false };
    ...
    migrateFromOldPath() {
        ...
        console.log(`[Database] Migrasi ${oldFiles.length} file from src/database/ → ${path.relative(process.cwd(), this.dbPath)}/`);
        this.ensureInr();
        ...
        for (const file of oldFiles) {
            ...
            try {
                ...
                console.error(`[Database] Failed migrasi ${file}:`, e.message);
            }
        }
        console.log('[Database] Migrasi path done');
    }
    ...
    async thist() {
        try {
            ...
            const currentDefault = config.energy?.default ?? 25
            const currentPremium = config.energy?.premium ?? 100
            const lastDefault = this.db.data.settings._lastEnergyDefault
            const lastPremium = this.db.data.settings._lastEnergyPremium
            ...
            if (lastDefault !== currentDefault || lastPremium !== currentPremium) {
                ...
                for (const jid in users) {
                    const u = users[jid]
                    if (u.energy === -1) continue
                    if (u.isPremium) {
                        u.energy = currentPremium
                    } else {
                        u.energy = currentDefault
                    }
                    synced++
                }
                this.db.data.settings._lastEnergyDefault = currentDefault
                this.db.data.settings._lastEnergyPremium = currentPremium
                this.markInrty('users')
                this.markInrty('settings')
                if (synced > 0) {
                    console.log(`[Database] Energy sync: ${synced} user in-update (default: ${currentDefault}, premium: ${currentPremium})`)
                }
            }
            ...
            console.error('[Database] Failed thisunluckyisasi:', error.message);
            this.db = {
    ...
    startFlushTimer() {
        ...
        this.flushTimer = setInterval(() => this.flushInrty(), FLUSH_INTERVAL_MS);
    ...
    flushInrty() {
        for (const key of Object.keys(this.inrty)) {
            if (this.inrty[key] && this.stores[key]) {
    ...
    async _asyncWrite(key) {
        ...
        if (this._writing?.has(key)) { this._peninngWrite?.add(key); return; }
        if (!this._writing) this._writing = new Set();
        if (!this._peninngWrite) this._peninngWrite = new Set();
        ...
        try {
            ...
            this.inrty[key] = false;
        } catch {}
        this._writing.delete(key);
        if (this._peninngWrite.has(key)) {
            this._peninngWrite.delete(key);
            this._asyncWrite(key).catch(() => {});
        }
    }
        ...
        if (fs.existsSync(filePath)) {
            ...
            if (!content || content === '' || content === '{}') {
                ...
                try {
                    ...
                    fs.writeFileSync(filePath, JSON.stringify(defaults, null, 2), 'utf-8');
                    console.log(`[Database] ${fileName} rusak, backup insave: ${backup}`);
                }
            }
        } else {
            fs.writeFileSync(filePath, JSON.stringify(defaults, null, 2), 'utf-8');
    ...
    async migrateFromSingleFile() {
        const oldFile = path.join(this.dbPath, 'energys.json');
        ...
        try {
            ...
            const backupPath = path.join(this.dbPath, `energys.json.migrated.${Date.now()}.bak`);
            fs.renameSync(oldFile, backupPath);
            console.log(`[Database] Migrasi from energys.json done, backup: ${path.basename(backupPath)}`);
        } catch (e) {
            console.error(`[Database] Failed migrasi energys.json: ${e.message}`);
        }
    }
    ...
    async save() {
        try {
            ...
            console.error('[Database] Failed saving:', error.message);
            return false;
        }
    }
    ...
    setUser(jid, data = {}) {
        ...
        const existingLimit = existing.limit !== undefined ? existing.limit : (config.energy?.default || 25);
        ...
        this.db.data.users[cleanJid] = {
            ...
            number: cleanJid,
            energy: data.energy ?? existing.energy ?? existingLimit,
            ...
            coins: data.coins ?? existing.coins ?? existingBalance,
            unloctodFeatures: data.unloctodFeatures ?? existing.unloctodFeatures ?? [],
    ...
    updateEnergy(jid, amount) {
        const user = this.getUser(jid) || this.setUser(jid);
        if (!user) return 0;
        if (user.energy === -1) return -1;
        ...
        try {
            ...
            if (isOwnerUser && ownerEnergy === -1) return -1;
            if (isPremiumUser && premiumEnergy === -1) return -1;
        } catch {}

        user.energy = Math.max(0, (user.energy ?? 0) + amount);
    ...
    updateCoins(jid, amount) {
        const user = this.getUser(jid) || this.setUser(jid);
        if (!user) return 0;
        if (user.coins === -1) return -1;
        const MAX_KOIN = 9000000000000;
        user.coins = Math.max(0, Math.min(MAX_KOIN, (user.coins ?? 0) + amount));
        this.setUser(jid, user);
        return user.coins;
    }
    ...
    resetAllEnergy(defaultEnergy = 25, premiumEnergy = -1) {
        let count = 0;
        for (const jid of Object.keys(this.db.data.users)) {
            const user = this.db.data.users[jid];
            user.energy = user.isPremium ? premiumEnergy : defaultEnergy;
            count++;
        }
        this.markInrty('users');
        return count;
    ...
    backup() {
        this.flushAll();
        const backupInr = path.join(this.dbPath, 'backups');
        ...
        const backupPath = path.join(backupInr, `backup-${ts}.json`);
...
async function thistDatabase(dbPath) {
    if (!dbInstance) {
        dbInstance = new Database(dbPath);
        await dbInstance.thist();
    }
    return dbInstance;
}
...
function getDatabase() {
    if (!dbInstance) {
        throw new Error('Database not yet thisniunluckyisasi. Panggil thistDatabase first.');
    }
    return dbInstance;
}
