...
const DO_DIR = path.join(process.cwd(), 'database', 'ingitalocean')
...
function addRole(jid, server, role) {
    if (!VALID_SERVERS.includes(server)) return { success: false, error: 'Server no valid' }
    if (!VALID_ROLES.includes(role)) return { success: false, error: 'Role no valid' }
    
    const number = cleanNumber(jid)
    if (!number) return { success: false, error: 'Number no valid' }
    
    const existingRole = getUserRole(jid, server)
    if (existingRole) {
        return { success: false, error: `Already has role ${existingRole} in ${server.toUpperCase()}` }
    }
...
function removeRole(jid, server, role) {
    ...
    if (idx === -1) {
        return { success: false, error: 'No registered' }
    }
