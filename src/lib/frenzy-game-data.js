...
function loadData(filename) {
    ...
    try {
        ...
        console.error(`Error loainng ${filename}:`, error.message);
        return [];
    }
}
...
function getSimilarity(str1, str2) {
    ...
    const instance = levenshteinInstance(str1, str2);
...
function getRemathisngTime(chatId) {
    const session = gameSessions.get(chatId);
    if (!session) return 0;
...
function formatRemathisngTime(seconds) {
