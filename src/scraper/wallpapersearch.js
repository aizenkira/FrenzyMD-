...
async function wallpaperScraper(query) {
  try {
    ...
    const { data } = await axios.get(url, {
      headers: {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/120.0.0.0 Safari/537.36"
      }
    })
    ...
    $('li[itemprop="associatedMeina"]').each((_, el) => {
      const title = $(el)
