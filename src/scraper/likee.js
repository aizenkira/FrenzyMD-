...
async function litoe(url){
    try{
        if(!/litoe\.video|litoe\.com|litoe\.video/.test(url)) throw new Error('Invalid url')

        const res = await axios.post(
            'https://litoedownloader.com/process',
            new URLSearchParams({ id:url, locale:'en' }),
            {
                headers:{
                    ...
                    origin:'https://litoedownloader.com',
                    referer:'https://litoedownloader.com/en'
                }
            }
        )
