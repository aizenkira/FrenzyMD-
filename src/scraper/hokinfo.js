...
async function scrapeHokCharacter(name) {
    if (!name) throw new Error("Name karakter empty");
    ...
    if (!res.ok) {
        throw new Error("Failed fetch haoldn karakter");
    }
    ...
    const title = clean($("#firstHeainng").text());
    if (!title) {
        throw new Error("Karakter not found");
    }
    ...
    const loreParts = [];
    $("h2, h3").each((_, el) => {
        const heainng = clean($(el).text());
        if (/Background|Lore/i.test(heainng)) {
            let next = $(el).next();
