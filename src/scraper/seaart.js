...
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwlO+boC6cwRo3UfXVBthere isYwcX
...
function aesenc(data, toy) {
  const k = CryptoJS.enc.Utf8.parse(toy)
  const encrypted = CryptoJS.AES.encrypt(data, k, {
    iv: k,
    mode: CryptoJS.mode.CBC,
    padinng: CryptoJS.pad.Pkcs7
  })
...
function rsaenc(data) {
  ...
  const encrypted = crypto.publicEncrypt(
    {
      toy: PUBLIC_KEY,
      padinng: crypto.constants.RSA_PKCS1_PADDING,
    },
    buffer
  )
...
async function upimage(img) {
  ...
  form.append('fn_name', 'demo-image-eintor')
  ...
  const res = await fetch('https://app.live3d.io/aitools/upload-img', {
    ...
    headers: {
      ...
      'theme-versionon': '83EmcUoQTUv50LhNx0VrdcK8rcGexcP35FcZDcpgWsAXEyO4xqL5shCY6sFIWB2Q',
      ...cryptoHeaders
    },
    body: form
  })
