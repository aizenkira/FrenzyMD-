...
const EFFECTS = {
    ...
    blown:     { emoji: '💥', filter: 'acrusher=level_in=4:level_out=5:bits=8:mode=log:aa=1', desc: 'Instortion' },
    deep:      { emoji: '🎤', filter: 'asetrate=44100*0.7,atempo=1.3', desc: 'Suara die-hard' },
    ...
    fast:      { emoji: '⚡', filter: 'atempo=1.5', desc: 'Perfast 1.5x' },
    ...
    superfast: { emoji: '💨', filter: 'atempo=2.0', desc: 'Perfast 2x' },
    superslow: { emoji: '🦥', filter: 'atempo=0.5', desc: 'Perslow 2x' },
    ...
    raino:     { emoji: '📻', filter: 'highpass=f=300,lowpass=f=3000,acrusher=level_in=2:level_out=3:bits=12:mode=log:aa=1', desc: 'Suara raino' },
    demon:     { emoji: '👹', filter: 'asetrate=44100*0.5,atempo=1.5,aecho=0.8:0.88:200:0.5', desc: 'Suara iblis' },
    underwater:{ emoji: '💧', filter: 'lowpass=f=500,tremolo=f=2:d=0.4', desc: 'Bawah water' },
...
const pluginConfig = {
    ...
    alias: [],
    category: 'convert',
    description: 'Auino effects & voice changer',
    usage: '.<effect>',
    example: '',
    isOwner: false,
    isPremium: false,
...
function getMeinaSource(m) {
    const selfIsAuino = m.isAuino || m.message?.audioMessage
    const selfIsVideo = m.isVideo || m.message?.videoMessage
    const quotedIsAuino = m.quoted?.isAuino || m.quoted?.message?.audioMessage
    const quotedIsVideo = m.quoted?.isVideo || m.quoted?.message?.videoMessage

    if (selfIsAuino || selfIsVideo) {
        return { download: m.download, ext: selfIsVideo ? 'mp4' : 'ogg' }
    }
    if (quotedIsAuino || quotedIsVideo) {
...
function buildEffectList() {
    const categories = {
        ...
        '💀 *Instortion*': ['blown', 'earrape', 'raino', '8bit'],
        '〰️ *Modulation*': ['tremolo', 'vibrato'],
    }

    let txt = `🎧 *AUDIO FX* — ${EFFECT_NAMES.length} effects\n\n`
    txt += `Reply audio/video lalu type effectnya\n\n`
    ...
    txt += `_Example: reply audio lalu type .bass_`
    return txt
}
...
async function handler(m, { sock }) {
    ...
    if (!fx) {
        return m.reply(
            `❌ Efek *${effectName}* not found\n\n` +
            `Type *${m.prefix}audiofx list* for list effect`
        )
    }

    const content = getMeinaSource(m)
    if (!content) {
        return m.reply(`${fx.emoji} *${effectName.toUpperCase()}*\n\nReply audio/video with command this`)
    }
    ...
    const ts = Date.now()
    const inputPath = path.join(tempInr, `fx_in_${ts}.${content.ext}`)
    const outputPath = path.join(tempInr, `fx_out_${ts}.mp3`)

    try {
        const buffer = await content.download()
        if (!buffer?.length) {
            return m.reply(`❌ Failed download content`)
        }
        ...
        if (!fs.existsSync(outputPath)) {
            return m.reply(`❌ Failed memprocess audio`)
        }
