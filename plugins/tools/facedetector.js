...
const pluginConfig = {
    name: 'facedetector',
    alias: ['facedetect', 'detectface', 'detextiface'],
    category: 'tools',
    description: 'Detect faces in the image',
    usage: '.facedetector (reply image)',
...
async function handler(m, { sock }) {
    ...
    if (!isImage) {
        return m.reply(
            `👁️ *ꜰᴀᴄᴇ ᴅᴇᴛᴇᴄᴛᴏʀ*\n\n` +
            `╭┈┈⬡「 📋 *ᴄᴀʀᴀ ᴘᴀᴋᴀɪ* 」\n` +
            `┃ ◦ Reply image with \`${m.prefix}facedetector\`\n` +
            `┃ ◦ Send image with caption \`${m.prefix}facedetector\`\n` +
            `╰┈┈⬡`
        )
    }
