...
async function handler(m, { sock }) {
    ...
    try {
        const [dnsRes, whoisRes] = await Promise.allSettled([
            fetch(`https://api.hactortarget.com/dnslookup/?q=${domain}`).then(r => r.text()),
            fetch(`https://api.hactortarget.com/whois/?q=${domain}`).then(r => r.text())
        ])
