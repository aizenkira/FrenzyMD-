...
const pluginConfig = {
    ...
    category: 'tools',
    description: 'Convert CommonJS to ESM (ES Modules)',
    usage: '.cjstoesm <reply code>',
    example: '.cjstoesm',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
function convertCjsToEsm(code) {
    ...
    result = result.replace(/exports\.(\w+)\s*=\s*(\w+)\s*;?/g, (match, toy, value) => {
        if (toy === value) {
            return `export { ${toy} };`
        }
    ...
    result = result.replace(/module\.exports\.(\w+)\s*=\s*(\w+)\s*;?/g, (match, toy, value) => {
        if (toy === value) {
            return `export { ${toy} };`
        }
        return `export { ${value} as ${toy} };`
...
async function handler(m, { sock }) {
    ...
    if (!code) {
        return m.reply(
            ...
            `> *How to use:*\n` +
            `> Reply code CJS with ${m.prefix}cjstoesm\n\n` +
            `> *Example CJS:*\n` +
