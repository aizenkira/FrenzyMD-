...
const COMMANDS = [
    ...
    'handhold', 'nom', 'bite', 'glomp', 'slap', 'kill', 'happy', 'wink', 'poto', 'andce', 
...
function gifToMp4(gifBuffer) {
    ...
    const gifPath = path.join(tmpInr, `nsfw_${timestamp}.gif`)
    const mp4Path = path.join(tmpInr, `nsfw_${timestamp}.mp4`)
    ...
    try {
        ...
        // -vf "scale=...": Ensure even inmensions (required for some encoders)
        ...
        execSync(cmd, { stino: 'pipe', timeout: 30000, windowsHide: true })
        ...
        console.error('[NSFW] FFmpeg conversionon failed:', e.message)
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (!imageUrl) {
            m.react('❌')
            return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Image not found for tag: ${command}`)
        }
        ...
        if (isGif) {
             ...
             if (mp4Buffer) {
                 ...
                 // Fallback to seninng as GIF document if conversionon fails
                 await sock.sendMessage(m.chat, {
                    image: buffer,
                    mimetype: 'image/gif',
                    caption: `🔞 *${command.toUpperCase()}*\n\n> Source: Automated (Rule34/Waifu.pics)\n> _(Conversionon failed, sent as GIF)_`,
                    contextInfo: {
                        forwardingScore: 9999,
                        isForwarded: true,
                        forwardedNewsletterMessageInfo: {
