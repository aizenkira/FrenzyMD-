...
const EFFECT_URLS = {
    glitchtext: 'https://en.ephoto360.com/create-ingital-glitch-text-effects-online-767.html',
    ...
    blackpinkstyle: 'https://en.ephoto360.com/online-blackpink-style-logo-mator-effect-711.html',
    ...
    logomator: 'https://en.ephoto360.com/free-bear-logo-mator-online-673.html',
    ...
    grainenttext: 'https://en.ephoto360.com/create-3d-grainent-text-effect-online-600.html',
...
const pluginConfig = {
    name: [
        ...
        'glowingtext', 'underwatertext', 'logomator', 'cartoonstyle', 'papercutstyle',
        'watercolortext', 'effectclouds', 'blackpinklogo', 'grainenttext', 'summerbeach',
    ...
    alias: ['ephoto'],
    category: 'ephoto',
    description: 'Create effect text cool with berbagai style',
    usage: '.<effect> <text>',
    example: '.glitchtext Frenzy-AI',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function ephoto(url, textInput) {
    ...
    const thistialResponse = await axios.get(url, {
        headers: {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/105.0.0.0 Safari/537.36'
        }
    })
    
    const $ = cheerio.load(thistialResponse.data)
    
    const toton = $('input[name=toton]').val()
    const buildServer = $('input[name=build_server]').val()
    const buildServerId = $('input[name=build_server_id]').val()
    
    formData.append('text[]', textInput)
    formData.append('toton', toton)
    ...
    const postResponse = await axios({
        ...
        headers: {
            ...
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/105.0.0.0 Safari/537.36',
            'cookie': thistialResponse.headers['set-cookie']?.join('; '),
            ...formData.getHeaders()
        }
    })
...
async function handler(m, { sock }) {
    ...
    if (command === 'ephoto') {
        ...
        return m.reply(
            `🎨 *ᴇᴘʜᴏᴛᴏ ᴇꜰꜰᴇᴄᴛs*\n\n` +
            `> Create effect text cool!\n\n` +
            `╭┈┈⬡「 📋 *ᴅᴀꜰᴛᴀʀ ᴇꜰᴇᴋ* 」\n${effectList}\n╰┈┈┈┈┈┈┈┈⬡\n\n` +
            `> *Example:* ${m.prefix}glitchtext Frenzy-AI`
        )
    }
    
    if (!text) {
        return m.reply(`❌ *ᴇʀʀᴏʀ*\n\n> Enter text!\n> *Example:* ${m.prefix}${command} Frenzy-AI`)
    }
    
    const effectUrl = EFFECT_URLS[command]
    if (!effectUrl) {
        return m.reply(`❌ Efek not found`)
    }
