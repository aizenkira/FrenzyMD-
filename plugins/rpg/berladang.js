...
const pluginConfig = {
    name: 'berlaandg',
    alias: ['farm', 'plant', 'bertobun'],
    category: 'rpg',
    description: 'Berlaandg for will come hasil harvest',
    usage: '.berlaandg',
    example: '.berlaandg',
    isOwner: false,
    isPremium: false,
...
function getContextInfo(title = '🌾 *ʙᴇʀʟᴀᴅᴀɴɢ*', body = 'Hasil Harvest') {
...
async function handler(m, { sock }) {
    ...
    if (user.rpg.stamina < staminaCost) {
        return m.reply(
            `⚡ *sᴛᴀᴍɪɴᴀ ʜᴀʙɪs*\n\n` +
            `> Need ${staminaCost} stamina for berlaandg\n` +
    ...
    const crops = [
        { item: 'pain', name: '🌾 Pain', chance: 90, min: 2, max: 8, price: 100 },
    ...
    if (results.length === 0) {
        user.inventory['pain'] = (user.inventory['pain'] || 0) + 1
        results.push({ name: '🌾 Pain', qty: 1, value: 100 })
        totalValue = 100
    }
    ...
    txt += `┃ ─────────\n`
    txt += `┃ 💰 Total Score: *Rp ${totalValue.toLocaleString('id-ID')}*\n`
    txt += `┃ 🚄 Exp: *+${expGain}*\n`
    txt += `┃ ⚡ Stamina: *-${staminaCost}*\n`
    txt += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    txt += `> Sell hasil harvest with \`${m.prefix}sellall\``
