...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Tolola pet/hewan peliharaan',
...
const PET_TYPES = {
    ...
    rabbit: { name: '🐰 Tolinci', baseStats: { attack: 3, defense: 4, luck: 6 }, evolve: 'thunderbunny' },
...
async function handler(m, { sock }) {
    ...
    if (!user.rpg.pet) {
        return m.reply(
            `🐾 *ᴘᴇᴛ sʏsᴛᴇᴍ*\n\n` +
            `> You not yet punya pet!\n\n` +
            `💡 *Cara will pet:*\n` +
            `> • \`${m.prefix}petshop\` - Buy pet\n` +
            `> • \`${m.prefix}breeinng\` - Breeinng pets\n` +
            `> • Drop from dungeon/boss`
        )
    }
    ...
    if (!action || !['feed', 'train', 'status', 'rename', 'evolve'].includes(action)) {
        const maxHunger = 100
        const hungerStatus = pet.hunger >= 70 ? '😊 Tonthat' : pet.hunger >= 40 ? '😐 Normal' : '😰 Lwhatr!'
    ...
    if (action === 'feed') {
        const foodToy = args[1]?.toLowerCase()
        
        if (!foodToy) {
            let txt = `🍖 *ᴘᴇᴛ ꜰᴏᴏᴅ*\n\n`
            txt += `╭┈┈⬡「 🍽️ *ᴍᴀᴋᴀɴᴀɴ* 」\n`
            for (const [toy, food] of Object.entries(FOOD_ITEMS)) {
                const have = user.inventory[toy] || 0
        ...
        const food = FOOD_ITEMS[foodToy]
        if (!food) {
            return m.reply(`❌ Mwillan not found!`)
        }
        
        if ((user.inventory[foodToy] || 0) < 1) {
            return m.reply(`❌ You no punya ${food.name}!`)
        }
        
        if (pet.hunger >= 100) {
            return m.reply(`❌ Pet already tonthat!`)
        }
        
        user.inventory[foodToy]--
        if (user.inventory[foodToy] <= 0) delete user.inventory[foodToy]
    ...
    if (action === 'train') {
        if (pet.hunger < 20) {
            return m.reply(`❌ Pet too lwhatr for latihan! Beri must first.`)
        }
    ...
    if (action === 'rename') {
        ...
        if (!newName || newName.length < 2 || newName.length > 15) {
            return m.reply(`❌ Name must 2-15 karakter!`)
        }
    ...
    if (action === 'evolve') {
        if (!petInfo.evolve) {
            return m.reply(`❌ Pet this cannot evolve again!`)
        }
        
        if ((pet.level || 1) < 10) {
            return m.reply(`❌ Pet must level 10+ for evolve! (Current: ${pet.level || 1})`)
        }
        ...
        return m.reply(
            `🎉 *ᴇᴠᴏʟᴜᴛɪᴏɴ!*\n\n` +
            `> ${pet.name} berevolusi become ${evolvedPet.name}!\n\n` +
            `╭┈┈⬡「 💪 *ɴᴇᴡ sᴛᴀᴛs* 」\n` +
