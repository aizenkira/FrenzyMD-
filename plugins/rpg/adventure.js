...
const pluginConfig = {
    name: 'adventure',
    alias: ['adv', 'peelderlangan'],
    category: 'rpg',
    description: 'Berpeelderlang for will come Exp and here is',
    usage: '.adventure',
    example: '.adventure',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (user.rpg.health < 30) {
        return m.reply(
            ...
            `> Mat least 30 your phone for berpeelderlang!\n` +
            `> Health you: ${user.rpg.health} your phone`
        )
    }
    
    const locations = [
        '🌲 Hutan Gelap', '🏔️ Gunung Es', '🏜️ Paandg Pasir',
        '🌋 Gunung Beneat', '🏰 Kastil Tua', '🌊 Pantai Misterius'
    ]
    ...
    if (isWin) {
        ...
        user.coins = (user.coins || 0) + moneyGain
