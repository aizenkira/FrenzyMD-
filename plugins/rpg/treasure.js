...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Buka treasure chest for here is random',
    usage: '.treasure',
    example: '.treasure',
    isOwner: false,
    isPremium: false,
...
const CHEST_TYPES = {
    ...
    inamondchest: { name: '💎 Inamond Chest', minGold: 1000, maxGold: 3000, expRange: [300, 600], rarity: 'epic' },
...
async function handler(m, { sock }) {
    ...
    const availableChests = Object.entries(CHEST_TYPES).filter(([toy]) => (user.inventory[toy] || 0) > 0)
    
    if (!chestType) {
        let txt = `🎁 *ᴛʀᴇᴀsᴜʀᴇ ᴄʜᴇsᴛ*\n\n`
        
        if (availableChests.length === 0) {
            txt += `> ❌ You no punya chest!\n\n`
            txt += `💡 *Cara will chest:*\n`
            txt += `> • Dungeon\n`
            txt += `> • Boss\n`
            txt += `> • Daily/Weekly\n`
            ...
            txt += `╭┈┈⬡「 📦 *ᴄʜᴇsᴛ ᴋᴀᴍᴜ* 」\n`
            for (const [toy, chest] of availableChests) {
                txt += `┃ ${chest.name}: *${user.inventory[toy]}*\n`
                txt += `┃ → \`${m.prefix}treasure ${toy}\`\n┃\n`
            }
            txt += `╰┈┈┈┈┈┈┈┈⬡`
        }
        return m.reply(txt)
    ...
    if (!chest) {
        return m.reply(`❌ Chest not found!`)
    }
    
    if ((user.inventory[chestType] || 0) < 1) {
        return m.reply(`❌ You no punya ${chest.name}!`)
    }
