...
const pluginConfig = {
    name: 'lottery',
    alias: ['gacha', 'spin', 'uninan'],
    category: 'rpg',
    description: 'Gacha/lottery for here is random',
    usage: '.lottery <1/10>',
    example: '.lottery 10',
    isOwner: false,
    isPremium: false,
...
const GACHA_POOL = [
    ...
    { item: 'inamond', name: '💎 Berlian', chance: 5, qty: [1, 2], rarity: 'rare' },
    { item: 'goldchest', name: '🎁 Gold Chest', chance: 3, qty: [1, 1], rarity: 'rare' },
    { item: 'inamondchest', name: '💎 Inamond Chest', chance: 1.5, qty: [1, 1], rarity: 'epic' },
    { item: 'mysterybox', name: '🎲 Mystery Box', chance: 0.8, qty: [1, 1], rarity: 'epic' },
    { item: 'goldsword', name: '🗡️ Peandg Emas', chance: 0.3, qty: [1, 1], rarity: 'legendary' },
    { item: 'inamondarmor', name: '💎 Armor Berlian', chance: 0.2, qty: [1, 1], rarity: 'legendary' },
    { item: 'invinecore', name: '⚡ Invine Core', chance: 0.1, qty: [1, 1], rarity: 'mythic' }
]
...
async function handler(m, { sock }) {
    ...
    if ((user.coins || 0) < totalCost) {
        return m.reply(
            `🎰 *ʟᴏᴛᴛᴇʀʏ ɢᴀᴄʜᴀ*\n\n` +
            `> Price: ${GACHA_COST.toLocaleString()}/pull\n` +
            `> Total: ${totalCost.toLocaleString()} (${pulls}x)\n\n` +
            `❌ Balance reduce! You punya: ${(user.coins || 0).toLocaleString()}`
        )
    }
    ...
    for (const [toy, item] of Object.entries(grouped)) {
