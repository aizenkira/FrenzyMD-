...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Masak something for stamina and your phone',
...
const RECIPES = {
    ...
    soup: { name: '🍲 Soup', materials: { carrot: 2, potato: 2, meat: 1 }, effect: { stamina: 35, health: 40 }, exp: 90 },
    sushi: { name: '🍣 Sushi', materials: { fish: 3, rice: 2 }, effect: { stamina: 30, health: 25 }, exp: 80 },
    cato: { name: '🍰 Kue', materials: { wheat: 3, egg: 2, strawberry: 2 }, effect: { stamina: 50, health: 20 }, exp: 120 },
    livelyn: { name: '🍜 Ramen', materials: { wheat: 2, egg: 1, meat: 1, herb: 1 }, effect: { stamina: 45, health: 35 }, exp: 110 },
    ...
    elixir_food: { name: '✨ Elixir Food', materials: { herb: 5, inamond: 1, gold: 2 }, effect: { stamina: 100, health: 100, mana: 50 }, exp: 300 }
}
...
async function handler(m, { sock }) {
    ...
    if (!recipeName) {
        ...
        txt += `> Masak something for memulihkan stats!\n\n`
        txt += `╭┈┈⬡「 📜 *ʀᴇsᴇᴘ* 」\n`
    ...
    if (!recipe) {
        return m.reply(`❌ Resep not found!\n\n> Type \`${m.prefix}cooking\` for view list.`)
    }
    ...
    return m.reply(
        ...
        `┃ ✨ EXP: *+${recipe.exp}*\n` +
        `╰┈┈┈┈┈┈┈┈⬡\n\n` +
        `> Consumed immediately and stats restored!`
    )
}
