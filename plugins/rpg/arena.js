...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Bertarung in arena PvP',
    usage: '.arena <@user>',
    example: '.arena @user',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (!mentioned) {
        return m.reply(
            `⚔️ *ᴀʀᴇɴᴀ ᴘᴠᴘ*\n\n` +
            `> Tantang other players for duel!\n\n` +
            `╭┈┈⬡「 📋 *ᴄᴀʀᴀ ᴘᴀᴋᴀɪ* 」\n` +
            `┃ ${m.prefix}arena @user\n` +
            `┃ Reply message user + ${m.prefix}arena\n` +
            `╰┈┈┈┈┈┈┈┈⬡\n\n` +
    ...
    if (mentioned === m.sender) {
        return m.reply(`❌ Cannot bertarung with self yourself!`)
    }
    
    const opponent = db.getUser(mentioned)
    if (!opponent) {
        return m.reply(`❌ Opponent not yet registered in database!`)
    }
    ...
    while (myHp > 0 && oppHp > 0 && round < 10) {
        ...
        oppHp -= myDmg
        battleLog.push(`🔥 You attack: *-${myDmg} your phone*`)
        
        if (oppHp <= 0) break
        
        const oppDmg = Math.max(5, oppAttack - myDefense + Math.floor(Math.random() * 10))
        myHp -= oppDmg
        battleLog.push(`💢 Opponent attack: *-${oppDmg} your phone*`)
    }
    ...
    txt += `╭┈┈⬡「 📊 *sᴛᴀᴛs* 」\n`
    txt += `┃ 🧑 You: ${Math.max(0, myHp)}/${myHealth} your phone\n`
    txt += `┃ 👤 Opponent: ${Math.max(0, oppHp)}/${oppHealth} your phone\n`
    txt += `┃ 🔄 Round: ${round}\n`
    txt += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    ...
    if (isWin) {
        ...
        const goldReward = Math.floor((opponent.coins || 0) * 0.1)
        
        user.coins = (user.coins || 0) + goldReward
        opponent.coins = Math.max(0, (opponent.coins || 0) - goldReward)
        ...
        const goldLoss = Math.floor((user.coins || 0) * 0.2)
