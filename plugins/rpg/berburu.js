...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Berburu hewan for will come item',
    usage: '.berburu',
    example: '.berburu',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    const animals = [
        { name: '🐰 Tolinci', item: 'daging_tolinci', chance: 80, min: 1, max: 3, exp: 50, money: 500 },
        ...
        { name: '🦊 Rchange', item: 'bulu_rchange', chance: 30, min: 1, max: 1, exp: 200, money: 3000 },
        { name: '🐻 Bermoney', item: 'cakar_bermoney', chance: 15, min: 1, max: 1, exp: 500, money: 10000 },
    ...
    if (caught.length === 0) {
        ...
        return m.reply(
            `🏹 *ʙᴇʀʙᴜʀᴜ ɢᴀɢᴀʟ*\n\n` +
            `> You no will come buruan times this\n` +
            `> Stamina: *-${staminaCost}*`
        )
    }
    ...
    user.coins = (user.coins || 0) + totalMoney
    ...
    txt += `┃ ─────────\n`
    txt += `┃ 💵 Sell: *+Rp ${totalMoney.toLocaleString('id-ID')}*\n`
    txt += `┃ 🚄 Exp: *+${totalExp}*\n`
