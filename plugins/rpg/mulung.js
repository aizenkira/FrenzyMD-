...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Memulung for mengumpulkan barang',
    usage: '.mulung',
    example: '.mulung',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (user.rpg.stamina < staminaCost) {
        return m.reply(
            `⚡ *sᴛᴀᴍɪɴᴀ ʜᴀʙɪs*\n\n` +
            `> Need ${staminaCost} stamina for memulung\n` +
    ...
    user.coins = (user.coins || 0) + moneyEarned
    ...
    txt += `┃ ─────────\n`
    txt += `┃ 💵 Sell: *+Rp ${moneyEarned.toLocaleString('id-ID')}*\n`
    txt += `┃ 🚄 Exp: *+${expGain}*\n`
