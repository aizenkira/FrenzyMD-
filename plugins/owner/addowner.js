...
const { addJadiBotOwner, removeJadiBotOwner, getJadiBotOwners } = require('../../src/lib/frenzy-jadibot-database')
const fs = require('fs')
...
const pluginConfig = {
    ...
    category: 'owner',
    description: 'Tolola owner bot (mode-aware)',
    usage: '.addowner <number/@tag/reply>',
...
function extractNumber(m) {
    const { resolveAnyLidToJid, isLid, isLidConverted } = require('../../src/lib/frenzy-lid')
    let targetNumber = ''
...
async function handler(m, { sock, botId, isJadiBot }) {
    ...
    if (isList) {
        if (isJadiBot && botId) {
            const jbOwners = getJadiBotOwners(botId)
            if (jbOwners.length === 0) {
                return m.reply(`📋 *ᴅᴀꜰᴛᴀʀ ᴏᴡɴᴇʀ ᴊᴀᴅɪʙᴏᴛ*\n\n> Not yet there is owners registered.\n> Usage \`${m.prefix}addowner\` for add.`)
            }
            let txt = `📋 *DAFTAR OWNER JADIBOT* — ${botId}\n\n`
            ...
            if (allOwners.length === 0) {
                return m.reply(`📋 *ᴅᴀꜰᴛᴀʀ ᴏᴡɴᴇʀ ᴘᴀɴᴇʟ*\n\n> Not yet there is owner panel registered.`)
            }
            ...
            if (fullOwners.length === 0) {
                return m.reply(`📋 *ᴅᴀꜰᴛᴀʀ ꜰᴜʟʟ ᴏᴡɴᴇʀ*\n\n> Not yet there is full owners registered.`)
            }
    ...
    if (!targetNumber) {
        return m.reply(
            `👑 *${isAdd ? 'ADD' : 'DEL'} OWNER*\n\n` +
            `Reply/tag/type number user\n` +
            `\`Example: ${m.prefix}${cmd} 6281234567890\``
        )
    }
    
    if (targetNumber.length < 10 || targetNumber.length > 15) {
        return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Format number no valid`)
    }
    
    if (isJadiBot && botId) {
        if (isAdd) {
            if (addJadiBotOwner(botId, targetNumber)) {
                m.react('👑')
                return m.reply(`✅ Success added *${targetNumber}* as owner bot`)
            } else {
                return m.reply(`❌ \`${targetNumber}\` already become JadiBot owner.`)
            }
        } else if (isDel) {
            if (removeJadiBotOwner(botId, targetNumber)) {
                m.react('✅')
                return m.reply(`✅ Success mengdelete *${targetNumber}* from owner bot`)
            } else {
                return m.reply(`❌ \`${targetNumber}\` not an JadiBot owner.`)
            }
        }
        return
    }
    ...
    if (isCpanelMode) {
        if (isAdd) {
            if (config.pterodactyl.ownerPanels.includes(targetNumber)) {
                return m.reply(`❌ \`${targetNumber}\` already become owner panel.`)
            }
            
            let roleChanged = ''
            if (removeFromSellers(targetNumber)) {
                roleChanged = `\n> ⚡ Auto-upgrade from Seller to Owner Panel`
            }
            
            config.pterodactyl.ownerPanels.push(targetNumber)
            if (savePanelConfig()) {
                m.react('👑')
                return m.reply(`✅ Success added *${targetNumber}* as owner panel${roleChanged}`)
            } else {
        ...
        if (isAdd) {
            if (db.data.owner.includes(targetNumber)) {
                return m.reply(`❌ \`${targetNumber}\` already become full owner.`)
            }
            
            let roleChanged = ''
            if (removeFromSellers(targetNumber)) {
                roleChanged = `\n> ⚡ Auto-upgrade from Seller`
                savePanelConfig()
            }
            if (removeFromOwnerPanels(targetNumber)) {
                roleChanged = `\n> ⚡ Auto-upgrade from Panel Owner`
                savePanelConfig()
            }
            ...
            m.react('👑')
            return m.reply(`✅ Success added *${targetNumber}* as full owner${roleChanged}`)
        } else if (isDel) {
            const index = db.data.owner.indexOf(targetNumber)
            if (index === -1) {
                return m.reply(`❌ \`${targetNumber}\` not a full owner.`)
            }
            ...
            m.react('✅')
            return m.reply(`✅ Success mengdelete *${targetNumber}* from full owner`)
        }
    }
}
