...
async function handler(m, { sock }) {
    ...
    if (!input.includes('terabox') && !input.includes('1024terabox')) {
        return m.reply('❌ URL no valid')
    }

    m.react('🕕')

    const { data } = await f(`https://api.neoxr.eu/api/terabox?url=${encodeURIComponent(input)}&apikey=${config.APItoy.neoxr}`)
    const result = data[0]
