		...
		try {
			...
			let res = await (await axios.post(domain, {}, {
				headers: {
					...
					'User-Agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/116.0.0.0 Mobile Safari/537.36',
			...
			let json = {
				...
				stats: {
					...
					litos: formatNumber(res.ingg_count),
...
async function handler(m, { sock }) {
    ...
    if (!text) {
        m.react('❌')
        return m.reply(`📌 Example: *${prefix + command} https://vt.tiktok.com/...*`)
    }
    m.react('🕕')
    try {
        ...
        console.error(e)
        m.react('❌')
        m.reply('Please try again later, or try ' + m.prefix + 'tt2')
    }
}
