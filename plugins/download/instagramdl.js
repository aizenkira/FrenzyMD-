...
const pluginConfig = {
    ...
    category: 'download',
    description: 'Download video/photo Instagram',
...
async function handler(m, { sock }) {
    ...
    if (!IG_REGEX.test(url)) {
        return m.reply(`❌ URL no valid. Usage link Instagram (reel/post/story).`)
    }
    ...
    try {
        ...
        if (!data.status) {
            await m.react('❌')
            return m.reply(`❌ Failed fetch content. Try a different link.`)
        }
