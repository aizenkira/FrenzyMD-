...
async function threadsdl(url) {
    ...
    const { data } = await axios.post(
        'https://threadsdownload.net/ms?fresh-pmeaningal=true',
        form,
        {
            headers: {
                accept: '*/*',
    ...
    if (end === -1) throw new Error('JSON no valid')
...
const pluginConfig = {
    ...
    category: 'download',
    description: 'Download photo Threads (album)',
    usage: '.tdl <url>',
...
async function handler(m, { sock }) {
    ...
    if (!url || !/threads/i.test(url)) {
        return m.reply(`❌ Usage URL Threads that is valid`)
    }
    ...
    try {
        ...
        if (images.length === 0) {
            throw new Error('No there is image intemukan')
        }
