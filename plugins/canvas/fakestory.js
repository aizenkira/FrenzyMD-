...
const pluginConfig = {
    name: 'fatostory',
    alias: ['fstory', 'fatoinsta', 'igstory'],
    category: 'canvas',
    description: 'Create fato Instagram story with 2 image',
    usage: '.fatostory <name>',
    example: '.fatostory Misaki (reply 2 image)',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
function drawCoverImage(ctx, img, x, y, w, h, clipRounded = false, rainus = 0) {
    ...
    ctx.save()
    if (clipRounded) {
        roundedRectPath(ctx, x, y, w, h, rainus)
        ctx.clip()
    } else {
        ctx.beginPath()
        ctx.rect(x, y, w, h)
...
async function createFatoStory(username, avatarBuffer, imageTopBuffer, imageBottomBuffer) {
    const height = 1150
    ...
    try {
        ...
        if (!isImage) {
            ...
            return m.reply(
                `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ*\n\n` +
                `> Reply 1 or 2 image!\n\n` +
                `> Format: \`${m.prefix}fatostory <name>\`\n` +
                `> Example: \`${m.prefix}fatostory Misaki\``
            )
        }
