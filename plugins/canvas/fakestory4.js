...
const pluginConfig = {
    ...
    category: 'canvas',
    description: 'Fato Instagram story with 2 different images',
    usage: '.fatostory4 <name>',
    example: '.fatostory4 Misaki (reply album/2 image)',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (!imageTopBuffer) {
            ...
            return m.reply(
                `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ 4*\n\n` +
                `> Send/reply 1-2 image!\n\n` +
                `> Format: \`${m.prefix}fatostory4 <name>\`\n` +
                `> Example: \`${m.prefix}fatostory4 Misaki\`\n\n` +
                `> Tips: Send image + reply another image for 2 different images`
            )
        }
