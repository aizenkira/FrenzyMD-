...
const pluginConfig = {
    ...
    category: 'canvas',
    description: 'Fato Instagram story with 1 image full',
    usage: '.fatostory2 <name>',
    example: '.fatostory2 Misaki (reply image)',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function createFatoStory(username, avatarBuffer, imageBuffer) {
    const height = 1150
    ...
    try {
        ...
        if (!isImage) {
            ...
            return m.reply(
                `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ 2*\n\n` +
                `> Reply image!\n\n` +
                `> Format: \`${m.prefix}fatostory2 <name>\`\n` +
                `> Example: \`${m.prefix}fatostory2 Misaki\``
            )
        }
        ...
        const resultBuffer = await createFatoStory(username, avatarBuffer, imageBuffer)
