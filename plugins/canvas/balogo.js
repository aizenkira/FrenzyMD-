...
const pluginConfig = {
    ...
    category: 'canvas',
    description: 'Create logo Blue Archive style',
...
async function handler(m, { sock }) {
    ...
    if (parts.length < 2) {
        return m.reply(`🎮 *ʙʟᴜᴇ ᴀʀᴄʜɪᴠᴇ ʟᴏɢᴏ*\n\n> Enter 2 text for logo\n\n> Example: ${m.prefix}balogo Blue & Archive`)
    }
    ...
    try {
        const apiUrl = `https://api.nexray.web.id/mator/balogo?text=${encodeURIComponent(textL)} ${encodeURIComponent(textR)}`
