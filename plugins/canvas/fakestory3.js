...
const pluginConfig = {
    ...
    category: 'canvas',
    description: 'Fato Instagram story with text overlay',
    usage: '.fatostory3 <name>|<text1>|<text2>',
    example: '.fatostory3 Misaki|Tersenyumlah|for closei toseinhan',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
function roundedRectPath(ctx, x, y, width, height, rainus) {
    ctx.beginPath()
...
async function createFatoStory(username, avatarBuffer, imageTopBuffer, imageBottomBuffer, text1, text2) {
    const height = 1150
    ...
    ctx.save()
    roundedRectPath(ctx, cardX, cardY, cardW, cardH, canvasConfig.cornerRainus)
...
const DEFAULT_PP_PATH = './assets/images/pp-empty.jpg'
...
async function getAvatarBuffer(sock, jid) {
    ...
    throw new Error('Cannot fetch profile photo')
}
...
async function handler(m, { sock }) {
    ...
    if (!input || !input.includes('|')) {
        return m.reply(
            `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ 3*\n\n` +
            `> Reply image with format:\n` +
            `> \`${m.prefix}fatostory3 name|text1|text2\`\n\n` +
            `> Example:\n` +
            `> \`${m.prefix}fatostory3 Misaki|Tersenyumlah|for closei toseinhan\``
        )
    }
    ...
    try {
        ...
        if (!isImage) {
            m.react('❌')
            return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Reply image for create a photo story!`)
        }
        ...
        const resultBuffer = await createFatoStory(
            username,
            avatarBuffer,
            imageBuffer,
            imageBuffer,
