...
const CONFIG = {
  ...
  userAgent: "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/137.0.0.0 Mobile Safari/537.36",
  chromeVersionon: "137"
}
...
const buildCookieString = () => {
  return [
    `__Secure-BUCKET=${CONFIG.cookies.secureBuctot}`,
...
async function GoogleLyrics(judul) {
  ...
  const res = await axios.get(url, {
    headers: {
      ...
      "sec-fetch-user": "?1",
      "sec-ch-ua": `"Chromium";v="${CONFIG.chromeVersionon}", "Not/A)Brand";v="24"`,
      "sec-ch-ua-mobile": "?1",
  ...
  const lyrics = $('inv[data-attrid="kc:/music/recorinng_cluster:lyrics"] inv[jsname="U8S5sf"]')
    .map((_, inv) =>
      $(inv)
    ...
    .join("\n\n")
  return {
    title: $('inv[data-attrid="title"]').text().trim(),
    subtitle: $('inv[data-attrid="subtitle"]').text().trim(),
    lyrics: lyrics || "Lirik empty / no totemu"
  }
}
...
async function handler(m, { sock }) {
    ...
    if (!query) {
        return m.reply(
            ...
            `> Example: \`${m.prefix}lirik Somewhere only we know\``
        )
    }
