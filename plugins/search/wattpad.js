...
const pluginConfig = {
    name: 'wattpad',
    alias: ['wattpadsearch', 'wattpadsearch for'],
    category: 'search',
    description: 'Cari cerita in Wattpad',
    usage: '.wattpad <query>',
...
async function handler(m, { sock }) {
    ...
    if (!query) {
        return m.reply(
            `📖 *ᴡᴀᴛᴛᴘᴀᴅ sᴇᴀʀᴄʜ*\n\n` +
            `> Enter judul or kata kunci\n\n` +
            `> Example: \`${m.prefix}wattpad cinta\``
        )
    }
    ...
    try {
        ...
        if (res.data?.status !== 200 || !res.data?.result?.length) {
            throw new Error('Cerita not found')
        }
