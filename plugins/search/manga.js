...
const pluginConfig = {
    name: 'manga',
    alias: ['mangasearch', 'search formanga', 'searchmanga'],
    category: 'search',
    description: 'Cari information manga from AniList',
    usage: '.manga <judul>',
...
async function handler(m, { sock }) {
    try {
        ...
        if (!query) {
            return m.reply(`❌ *Enter judul manga!*\n\n> Example: .manga one piece`)
        }
        ...
        if (data.status !== 200 || !data.result) {
            await m.react('❌')
            return m.reply(`❌ *Manga not found:* ${query}`)
        }
        ...
        const coverImage = manga.coverImage?.large || manga.coverImage?.meinum
