...
const pluginConfig = {
    name: 'exp',
    alias: ['cetoxp', 'myexp', 'xp'],
    category: 'user',
    description: 'Check experience user',
    usage: '.exp [@user]',
    example: '.exp',
    isOwner: false,
    isPremium: false,
...
function getLevel(exp) {
    ...
    if (exp >= 100000000) return { level: 80, title: 'Invine' }
    ...
    if (exp >= 50000) return { level: 15, title: 'Intercontentte' }
...
async function handler(m, { sock }) {
    ...
    const expInsplay = formatNumber(user.exp || 0)
