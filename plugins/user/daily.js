...
const { getTimeGreeting } = require('../../src/lib/frenzy-formatter')

const pluginConfig = {
    name: 'daily',
    alias: ['claim', 'daily', 'bonus'],
    category: 'user',
    description: 'Claim here daily (Exp, Money, Potion)',
    usage: '.daily',
    example: '.daily',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    txt += `╭┈┈⬡「 🎁 *ʀᴇᴡᴀʀᴅs* 」\n`
    txt += `┃ 🚄 Exp: *+${expReward}*\n`
    txt += `┃ 💰 Coins: *+${moneyReward.toLocaleString('id-ID')}*\n`
    txt += `┃ 🥤 Potion: *+${potionReward}*\n`
    txt += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    txt += `> Don't forget claim again tomorrow!`
