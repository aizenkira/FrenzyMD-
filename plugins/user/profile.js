...
const pluginConfig = {
    ...
    category: 'user',
    description: 'Meview profil user with RPG stats',
    usage: '.profile [@user]',
    example: '.profile',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        const fallbackPath = './assets/images/pp-empty.jpg'
        if (fs.existsSync(fallbackPath)) {
            ppMeina = fs.readFileSync(fallbackPath)
        } else {
            ppMeina = { url: 'https://i.imgur.com/TuItj4L.png' }
        }
    }

    let caption = `*〔 👤 USER PROFILE 〕*\n\n`

caption += `*〔 👤 Name 〕* ${user.name || m.pushName || 'User'}\n`
...
caption += `*〔 💰 ASSETS 〕*\n`
caption += `*〔 🪙 Coins 〕* Rp ${user.coins?.toLocaleString('id-ID') || 0}\n`
caption += `*〔 🏦 Bank 〕* Rp ${user.rpg.bank?.toLocaleString('id-ID') || 0}\n`
caption += `*〔 ⚡ Energy 〕* ${isOwner || isPremium ? '∞ Unlimited' : user.energy}\n`
