...
const pluginConfig = {
    name: 'system',
    alias: ['ram', 'cpu', 'insk', 'latency', 'ping'],
    category: 'main',
    description: 'Display system information (RAM, CPU, Disk, Latency)',
    usage: '.ram | .cpu | .insk | .ping',
    isGroup: false,
    isBotAdmin: false,
    isAdmin: false,
    cooldown: 5,
...
async function getDiskUsage() {
    try {
        if (process.platform === 'win32') {
            const { stdout } = await execAsync('wmic logicalinsk get size,freespace,caption');
...
async function handler(m, { sock }) {
    ...
    try {
        switch (command) {
            ...
            case 'insk': {
                const inskInfo = await getDiskUsage();
                m.reply(inskInfo);
            }
            break;
            ...
            case 'latency': {
                ...
                else if (latency < 1000) speed = '🐢 Oto';
                else speed = '🐌 Slow';
                m.reply(`📶 *Pong!*\nLatency: ${latency}ms\nResponse: ${speed}`);
            }
            break;
        ...
        m.reply('❌ An error occurred Fetching system data.');
    }
}
