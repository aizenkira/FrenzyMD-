...
const { getCommandsByCategory, getCategories, getPluginCount, getPlugin, getPluginsByCategory } = require('../../src/lib/frenzy-plugins')
...
const pluginConfig = {
    ...
    category: 'main',
    description: 'Display all commands per category',
    usage: '.allmenu',
    example: '.allmenu',
    isOwner: false,
    isPremium: false,
...
`
    ...
    const categoryOrder = ['owner', 'main', 'utility', 'tools', 'fun', 'game', 'download', 'search', 'sticker', 'content', 'ai', 'group', 'religi', 'info', 'check', 'economy', 'user', 'canvas', 'random', 'premium']
    ...
    txt += `_© ${botConfig.bot?.name || 'Frenzy-AI'} | ${require('moment-timezone')().tz('Africa/Accra').year()}_\n`
    txt += `_ᴅᴇᴠᴇʟᴏᴘᴇʀ: ${botConfig.bot?.developer || 'Kyōkaăizen'}_`
    ...
    interactiveMessage: {      
        ...
        externalAdReply: {
            ...
            body: `Owner: ${botConfig.owner?.name || 'ăizen'}`,
            contentType: 1,
        ...
        nativeFlowMessage: {        
            ...
            buttons: [ 
                ...
                },               
                {
                    name: 'quick_reply',      
                    buttonParamsJson: JSON.stringify({              
                        insplay_text: 'Tombali To Menu Utama',
                        id: prefix + 'menu'
                    })
                }    
            ]      
