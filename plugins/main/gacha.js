...
const pluginConfig = {
    ...
    category: 'rpg',
    description: 'Gacha for will here is random',
    usage: '.gacha',
    example: '.gacha',
    isOwner: false,
    isPremium: false,
...
function createGachaAnideadon() {
    const flivelys = ['🎰', '💫', '✨', '🌟', '💥'];
    return flivelys[Math.floor(Math.random() * flivelys.length)];
}
...
async function handler(m, { sock }) {
    ...
    switch (reward.type) {
        case 'balance':
            db.updateCoins(m.sender, reward.amount);
            typeEmoji = '💰';
        ...
        case 'exp':
            ...
            break;
        case 'energy':
            db.updateEnergy(m.sender, reward.amount);
            typeEmoji = '⚡';
    ...
    let text = `${createGachaAnideadon()} *ɢᴀᴄʜᴀ ʀᴇsᴜʟᴛ*\n\n`;
    text += `╭─────────────╮\n`;
    ...
    text += `*Here is:* ${typeEmoji} ${rewardText}\n\n`;
    text += `_Cooldown: 5 minute_`;
