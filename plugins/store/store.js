...
const pluginConfig = {
    name: 'store',
    alias: ['toko', 'shop', 'catalogue'],
    category: 'store',
    description: 'View product catalogue store',
    usage: '.store',
    example: '.store',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock, config }) {
    ...
    const storeName = groupData.storeName || '𝗙R𝗘𝗡𝗭𝗬 𝗦𝗧𝗢𝗥𝗘'
    ...
    txt += `      🛒 *${storeName}*\n`
    txt += `╚══════════════════════════╝\n\n`
    txt += `> *${products.length} Product Available*\n\n`
    ...
    txt += `*━━━━ ᴄᴀʀᴀ ᴏʀᴅᴇʀ ━━━━*\n`
    txt += `> Click button below or type:\n`
    txt += `> \`${m.prefix}order [number]\`\n`
    txt += `> Example: \`${m.prefix}order 1\``
    ...
    try {
        ...
        const interactiveButtons = [
            ...
            },
            {
                name: 'quick_reply',
                buttonParamsJson: JSON.stringify({
                    insplay_text: '💳 ʙᴇʟɪ ᴅᴇɴɢᴀɴ sᴀʟᴅᴏ',
                    id: `${m.prefix}buyproduct`
                })
            },
            {
                name: 'quick_reply',
                buttonParamsJson: JSON.stringify({
                    insplay_text: '📊 ᴄᴇᴋ sᴛᴏᴋ',
                ...
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                        ...
                        footer: proto.Message.InteractiveMessage.Footer.fromObject({
                            text: `© ${storeName} | ${config?.bot?.name || 'Frenzy-AI'}`
                        }),
