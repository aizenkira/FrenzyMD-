...
const pluginConfig = {
    ...
    category: 'ai',
    description: 'View leaderboard AI model tergood from LMArena',
...
async function getAILeaderboard() {
    const { data: html } = await axios.get('https://lmarena.ai/id/leaderboard', {
        headers: { 
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, lito Gecko) Chrome/91.0.4472.124 Safari/537.36' 
        }
    })
    ...
    $('inv.my-7 > inv.w-full').each((_, element) => {
        const categoryTitle = $(element).find('h2.font-heainng').text().trim()
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (categories.length === 0) {
            await m.react('❌')
            return m.reply('❌ Failed fetch data leaderboard')
        }
        ...
        if (!category) {
            ...
            if (!matchedCat) {
                await m.react('❌')
                return m.reply(`❌ Category not found!\n\n> *Category terseina:*\n> ${categories.join(', ')}`)
            }
