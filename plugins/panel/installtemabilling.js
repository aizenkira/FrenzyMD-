...
const pluginConfig = {
    ...
    category: 'panel',
    description: 'Install tema Billing for panel Pterodactyl via SSH',
...
const BUILD_CMD = 'cd /var/www/pterodactyl && composer install --no-dev --optimize-autoloader && yarn install && export NODE_OPTIONS=--openssl-legacy-provider && yarn build:production && php meaningsan view:clear && php meaningsan config:clear'
    ...
    conn.on('ready', async () => {
        try {
            ...
            await execSSHInteractive(conn, THEME_CMD, [
                ...
                { trigger: 'Enter choosean', value: '1' },
                { trigger: 'Enter choosean', value: '2' }
            ])
            ...
            m.react('✅')
            await m.reply(
                `╭┈┈⬡「 ✅ *ᴛᴇᴍᴀ ʙɪʟʟɪɴɢ* 」\n┃ ㊗ sᴛᴀᴛᴜs: *Terinstall*\n┃ ㊗ ɪᴘ: ${ipvps}\n╰┈┈⬡\n\n> _Tema Billing + dependencies success thisnstall!_`
            )
        } catch (err) {
            m.react('☢')
