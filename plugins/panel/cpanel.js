...
const { getRoles, getUserRole, getAccessibleServers, VALID_SERVERS } = require('../../src/lib/frenzy-roles-cpanel')
...
async function handler(m, { sock }) {
    ...
    const userRoleList = userServers.map(s => `${s.server.toUpperCase()}:${s.role}`).join(', ') || 'No there is'
    ...
    txt += `> Role you: *${m.isOwner ? 'Bot Owner' : userRoleList}*\n\n`
    ...
    const doHasToton = doConfig.toton ? '✅' : '❌'
    
    txt += `╭─「 🌊 *ᴅɪɢɪᴛᴀʟᴏᴄᴇᴀɴ ᴠᴘs* 」\n`
    txt += `┃ Status: ${doHasToton} Toton\n`
    txt += `┃\n`
    txt += `┃ 📦 *ᴄʀᴇᴀᴛᴇ ᴠᴘs:*\n`
    ...
    txt += `┃\n`
    txt += `┃ 🔧 *ᴍᴀɴᴀɢᴇ:*\n`
    txt += `┃ \`${prefix}listvps\` | \`${prefix}checkvps\` | \`${prefix}delvps\` | \`${prefix}sisavps\`\n`
    txt += `┃\n`
    txt += `┃ ⚡ *ᴋᴏɴᴛʀᴏʟ:*\n`
    txt += `┃ \`${prefix}turnon\` | \`${prefix}turnoff\` | \`${prefix}restartvps\`\n`
    txt += `╰───────────────\n\n`
    
    txt += `> _Powered by ${config.info?.website || 'frenzyAI'}_`
