...
const pluginConfig = {
    ...
    category: 'panel',
    description: 'Install tema Enigma for panel Pterodactyl via SSH',
...
async function handler(m) {
    ...
    if (parts.length < 5) {
        return m.reply(`❌ Wrong format!\n\n> Usage: \`ip|password|link_wa|link_group|link_channel\``)
    }
    ...
    conn.on('ready', async () => {
        try {
            ...
            await execSSHInteractive(conn, THEME_CMD, [
                ...
                { trigger: 'Enter choosean', value: '3' },
            ...
            m.react('✅')
            await m.reply(
                `╭┈┈⬡「 ✅ *ᴛᴇᴍᴀ ᴇɴɪɢᴍᴀ* 」\n┃ ㊗ sᴛᴀᴛᴜs: *Terinstall*\n┃ ㊗ ɪᴘ: ${ipvps}\n╰┈┈⬡\n\n> _Tema Enigma + dependencies success thisnstall!_`
            )
        } catch (err) {
            m.react('☢')
