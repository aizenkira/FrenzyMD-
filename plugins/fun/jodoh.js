...
const pluginConfig = {
    ...
    category: 'fun',
    description: 'Jodohkan 2 member random with tococokan',
    usage: '.match/soulmate',
    example: '.match/soulmate',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
const loveQuotes = [
    'Cinta sejati no pernah mengenal jarak 💕',
    'Dua heart that bersatu takkan terpisahkan 💗',
    'You two like puzzle that perfect 🧩',
    'Match made in heaven! ✨',
    'Chemistry-nya strong very! 🔥',
    'Couple goals very sih everyone 💑',
...
const compatibilityText = (percent) => {
    ...
    if (percent >= 70) return 'Very Cocok! 💖'
    if (percent >= 50) return 'Pretty Cocok 💗'
    if (percent >= 30) return 'Worth a try 💓'
    return 'Need Usaha Lebih 💔'
}
...
async function handler(m, { sock }) {
    ...
    if (memberJids.length < 2) {
        return m.reply('❌ *ɢᴀɢᴀʟ*\n\n> Mat least there is 2 member for inmatch/soulmatap!')
    }
    ...
    text += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    if (usedRegistration) {
        text += `> ✨ _Inmatch/soulmatap berdasarkan data registrasi_\n`
    }
    text += `> _"${quote}"_`
