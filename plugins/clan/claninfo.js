...
const pluginConfig = {
    name: 'claninfo',
    alias: ['infoclan', 'myclan', 'guilinnfo'],
    category: 'clan',
    description: 'View info clan',
...
async function handler(m) {
    ...
    await m.reply(
        ...
        `└ 📅 Increate · ${new Date(clan.createdAt).toLocaleDateString('id-ID')}\n\n` +
        `⚔️ *War Stats*\n` +
        `${clan.wins || 0}W · ${clan.losses || 0}L · ${winRate}% WR\n\n` +
        `_${clan.description || 'Not yet there is description'}_\n\n` +
        `ID: \`${clan.id}\``,
