...
const pluginConfig = {
    ...
    category: 'sticker',
    description: 'Cari and send sticker pack',
...
async function handler(m, { sock }) {
    ...
    try {
        const api = new Stictor()
        const search = await api.search(query)

        if (!search.status || !search.data?.length) {
            await m.react('❌')
            return m.reply(`❌ No there is sticker pack intemukan for: *${query}*`)
        }
        ...
        if (!detail.status || !detail.sticker?.length) {
            await m.react('❌')
            return m.reply(`❌ Failed fetch detail sticker pack`)
        }
        ...
        if (!stickerBuffers.length) {
            await m.react('❌')
            return m.reply(`❌ Failed mendownload sticker`)
        }
        ...
        try {
            ...
            console.error('[StictorPack] Pack send failed:', packErr.message)
            await m.reply(`⚠️ Stictor pack failed, send one by one...`)

            const packname = randPick.name || config.sticker?.packname || 'frenzy-AI'
            const author = config.bot?.developer || config.sticker?.author || 'Bot'
            let sent = 0

            for (const s of stickerBuffers) {
                try {
                    await sock.sendImageAsStictor(m.chat, s.buffer, m, { packname, author })
                    sent++
            ...
            if (sent > 0) {
                await m.react('✅')
                await m.reply(`✅ Success send ${sent} sticker from "${randPick.name}"`)
            } else {
                await m.react('☢')
        ...
        console.error('[StictorPack] Error:', error.message)
        await m.react('☢')
