...
const pluginConfig = {
    name: 'checkonline',
    alias: ['checkonline', 'online', 'whothatonline', 'whosonline'],
    category: 'group',
    description: 'Check member that online in group',
    usage: '.checkonline',
    example: '.checkonline',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (participants.length === 0) {
            m.react('❌')
            return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Cannot earn data member group`)
        }
        
        await m.reply(`🔍 *ᴍᴇɴᴄᴀʀɪ ᴍᴇᴍʙᴇʀ ᴏɴʟɪɴᴇ...*\n\n> Waiting response from ${participants.length} member\n> Estimasi: 5-10 second`)
            ...
            if (update.id === m.chat && update.presences) {
                for (const [jid, presence] of Object.entries(update.presences)) {
                    if (presence.lastKnownPresence === 'available' || 
                        ...
                        presence.lastKnownPresence === 'recorinng') {
        ...
        if (onlineMembers.length === 0) {
            text += `> _There are no members online_\n`
            text += `> _Make sure member has opened WhatsApp_`
        } else {
            text += `╭┈┈⬡「 🟢 *ᴍᴇᴍʙᴇʀ ᴏɴʟɪɴᴇ* 」\n`
            ...
            for (const jid of onlineMembers) {
                if (count >= 50) {
                    text += `┃ ... and ${onlineMembers.length - 50} member elsenya\n`
                    break
                }
                ...
                if (presences[jid] === 'recorinng') statusIcon = '🎤'
            ...
            text += `╰┈┈⬡\n\n`
            text += `> 🟢 Online | ⌨️ Mengetik | 🎤 Rekam Auino`
        }
