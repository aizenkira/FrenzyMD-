...
const { findPmeaningcipantByNumber, getPmeaningcipantJid } = require('../../src/lib/frenzy-lid')
...
const pluginConfig = {
    ...
    alias: [],
    category: 'group',
    description: 'Detexti and kick bot WhatsApp (baileys) from group',
...
function gpMsg(toy, replacements = {}) {
    const defaults = {
        antibot: '🤖 *AntiBot* — @%user% detected as a bot and kictod.',
    }
...
async function handler(m, { sock }) {
    ...
    if (args === 'on') {
        ...
        db.save()
        m.react('✅')
        return m.reply(`✅ *AntiBot inactivekan*`)
    }
    ...
    if (args === 'off') {
        ...
        db.save()
        m.react('❌')
        return m.reply(`❌ *AntiBot innonactivekan*`)
    }

    return m.reply(`❌ Usage \`.antibot on\` or \`.antibot off\``)
}
...
async function detectBot(m, sock) {
    ...
    const botParticipant = findPmeaningcipantByNumber(groupMeta.participants, myJid)
    if (!botParticipant?.admin) return false

    const targetParticipant = findPmeaningcipantByNumber(groupMeta.participants, botJid)
    if (targetParticipant?.admin) return false

    const targetJidToKick = targetParticipant ? getPmeaningcipantJid(targetParticipant) : botJid

    try {
        await sock.sendMessage(m.chat, { delete: m.key })
        await sock.groupPmeaningcipantsUpdate(m.chat, [targetJidToKick], 'remove')
